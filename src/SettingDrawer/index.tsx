  ...
  if (!hideMessageLoading) {
    hide = message.loading(
      formatMessage({
        ...
        defaultMessage: 'Cargando tema',
      }),
    );
  }

  const href = dark ? `${publicPath}/dark` : `${publicPath}/`;
  // If it is dark and color=daybreak, no stitching is required
  let colorFileName =
    dark && color
...
const initState = (
  ...
  if (window.location.search) {
    ...
    // If the theme is set in the url, load it once。
  ...
  // Si el tema no está configurado en la url y el que está en la url no está cargado, cárgarlo.
...
const getParamsFromUrl = (settings?: MergerSettingsType<ProSettings>) => {
  ...
  // La primera vez que ingresas y sincronizas con el navegador Parámetro
  let params = {};
  if (window.location.search) {
...
/**
 * Componentes de configuración visual.
 * @param props
 */
const SettingDrawer: React.FC<SettingDrawerProps> = (props) => {
  ...
  useEffect(() => {
    // Modificación de idioma, esto se configura con locale
    ...
    // Recuerde la opción predeterminada, fácil de hacer diff, y luego guardar en url Parámetro
    oldSetting = {
      ...defaultSettings,
      ...propsSettings,
    };
    ...
    /**
     * Si no es un navegador, no hay necesidad de hacerlo
     */
    if (!isBrowser()) {
      return () => null;
    }
