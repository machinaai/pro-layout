...
export interface BaseMenuProps
  ...
  /**
   * Para dar los accesorios del Menú, consulte las propiedades de antd-menu. https://ant.design/components/menu-cn/
   */
  menuProps?: MenuProps;
...
class MenuUtil {
  ...
  /**
   * Determine si se trata de un enlace http. Enlace de retorno o "a"
  ...
  getMenuItemPath = (item: MenuDataItem, isChildren: boolean) => {
    ...
    if (menuItemRender) {
      ...
      // Si hideChildrenInMenu elimina los hijos inútiles
...
/**
 * Genere el objeto openKeys, porque openKeys está configurado, se controlará, por lo que se necesita un objeto vacío
 * @param BaseMenuProps
 */
...
const BaseMenu: React.FC<BaseMenuProps> = (props) => {
  ...
  // Componentes utilizados para reducir los cálculos defaultOpenKeys
  ...
  /**
   * Necesita usar aquí menuData
   * Para calcular splitMenus necesitas usar el más completo menuData
   */
  useEffect(() => {
  ...
  // Esta vez cuando openKeys === false, en este caso, ayuda al usuario a seleccionar una vez
  // No importa después de la segunda vez, por lo que se usa defaultOpenKeys
