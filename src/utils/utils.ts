...
/** Determinar si es un enlace de imagen */
...
export const getKeyByPath = (item: MenuDataItem) => {
  ...
  // Si hay un nombre, use el nombre
  if (name) {
    return name;
  }
  // Si aún no lo hace, use el hash del objeto para generar uno
  try {
    return hash.sha256().update(stringify(item)).digest('hex');
  } catch (error) {
    // dom some thing
  }
  // Si aún no funciona, devuelve un valor aleatorio
  return guid();
};
